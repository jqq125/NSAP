...
def parse_adjlist_context(adjlist, edge_metapath_indices, samples, connection,device, offset_list, mode,sampled_idxs):
    cedges = []
    #cnodes:记录batch中所有药物/疾病节点的上下文结点
    cnodes= set()
    virtue_idx_lists=[]
    #使用大于等于dim的编号来记录虚拟结点
    tmp=sum(offset_list)
    ...
    if mode==0:
        #上下文节点(dst)加offset2，上下文节点的邻居节点(src)加offset1
    ...
    for row, indices,sampled_idx in zip(adjlist, edge_metapath_indices,sampled_idxs):
        row_parsed = list(map(int, row.split(' ')))
        #current_neighbors:存放当前节点（batch中的一个节点）的邻居
        ...
        #clists:存放当前的上下文节点，即current_neighbors中的所有节点
        clists=list(current_neighbors)
        clists.sort()
        # 创建虚拟结点,并将其与当前所有的上下文节点相连
        virtue_node = tmp
        tmp += 1
        ...
        # 为当前上下文节点添加一阶邻居
        #为当前上下文结点选取一阶邻居，药物则选取靶标，疾病则选取基因
        for dst in clists:
            if dst in cnodes:
                continue
        ...
        # virtue_idx_lists：存放所有的虚拟节点（其个数与batchsize的大小相同）
...
#     for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_ua, edge_metapath_indices_list_ua)):
#         for adjlist, indices in zip(adjlists, edge_metapath_indices_list):
#             edges, result_indices, num_nodes, mapping1 = parse_adjlist_meta(
#                 [adjlist[row[mode]] for row in drug_dis_batch],
#                 [indices[row[mode]] for row in drug_dis_batch], samples, offset_list, mode)
#             #获取元图
#             metaGraph = dgl.DGLGraph()
...
#             idx_batch_mapped_lists[mode].append(np.array([mapping1[row[mode]] for row in drug_dis_batch]))
#
#     return metaGraph_lists, result_indices_lists, idx_batch_mapped_lists
#
#
#
# def parse_adjlist_context(adjlist, edge_metapath_indices, samples, connection,device, offset_list, mode):
#     cedges = []
#     #cnodes:记录batch中所有药物/疾病节点的上下文结点
#     cnodes= set()
#     virtue_idx_lists=[]
...
#
#
# def parse_minibatch_context(adjlists_ua, edge_metapath_indices_list_ua, drug_dis_batch, device,connection_list,samples=None,offset_list=None):
#     contextGraph_lists = [[], []]
#     cnodes_lists=[[],[]]
#     vm_idx_lists=[[],[]]
#     for mode, (adjlists, edge_metapath_indices_list,connection) in enumerate(zip(adjlists_ua, edge_metapath_indices_list_ua,connection_list)):
#         #print("context mode:", mode)
#         for adjlist, indices in zip(adjlists, edge_metapath_indices_list):
#             # print(len(adjlist))
#             #print("t2:", time.time())
#             cedges,  cnodes, vm_idx_list,contextGraph = parse_adjlist_context(
#                 [adjlist[row[mode]] for row in drug_dis_batch],
#                 [indices[row[mode]] for row in drug_dis_batch], samples, connection,device,offset_list, mode)
#             #print("t3:", time.time())
#             contextGraph_lists[mode].append(contextGraph)
#             real_cnodes=cnodes[:-len(drug_dis_batch)]
#             cnodes = torch.tensor(real_cnodes).to(device)
